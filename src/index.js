// Arquivo: src/index.js (Sua Versão Final 100% Completa com a correção do CORS e a rota /minha-rede)
...
app.get('/minha-rede', protect, async (req, res) => {
    try {
        ...
        const direct = directReferrals.map(user => ({
            id: user.id,
            name: user.name,
            email: user.email,
        ...
        const indirect = directReferrals.filter(user => user.referrals.length > 0)
            .map(user => ({
                id: user.id,
                name: user.name,
                subAffiliates: user.referrals.map(subUser => ({
                    id: subUser.id,
                    name: subUser.name,
                    email: subUser.email,
        ...
        res.status(200).json({ direct, indirect });
    } catch (error) {
        console.error('Erro ao buscar dados da rede:', error);
        res.status(500).json({ error: 'Não foi possível carregar os dados da rede.' });
    }
});
